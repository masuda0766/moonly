				...
				mCheckStateTimerTask = new TimerTask() {
					...
					public void run() {
						...
						// get wlan band capability response
							...
							int wlan2GEnabled = mBTConfig.getWlanBand_2G();
							int wlan5GEnabled = mBTConfig.getWlanBand_5G();
							if(wlan2GEnabled != 1 && wlan5GEnabled != 1){
								...
								mWaitCountsForScanWlan2G = 0;
								Cmd4BTConfigThread.sendCommonCmd(BTConfigThread.CMD_START_SCAN_WLAN_2G, 
										MainActivity.mMainAct.mBTConfigHandler);
							}
							else if(wlan5GEnabled == 1){
								mWaitCountsForScanWlan5G = 0;
								Cmd4BTConfigThread.sendCommonCmd(BTConfigThread.CMD_START_SCAN_WLAN_5G, 
										MainActivity.mMainAct.mBTConfigHandler);
							}
							return;
						...
						//scan wlan AP 5G
							...
							if( mWaitCountsForScanWlan5G > mMaxWaitCountsForScan5G){
								...
								/* scan 2.4G+5G all timeout */	
								if(mBTConfig.getWlanScanResults(0).isEmpty() && mBTConfig.getWlanScanResults(1).isEmpty()){
									MainActivity.mMainAct.mUIHandler.sendEmptyMessage(UIMsgHandler.MSG_SHOW_SCAN_TIMEOUT);
									return;
								}
							...
							if(mWaitCountsForAckAPConnect > mMaxWaitCountsForAckAPConnect){
								...
								if(MainActivity.mMainAct.mRemoteAPBuf == null)
									return;
									 ...
									 Cmd4BTConfigThread.sendAPProfileCmd(MainActivity.mMainAct.mRemoteAPBuf, 
											 MainActivity.mMainAct.mBTConfigHandler);
								}
							}
						...
						//reply for check repeater status
							...
							if(!MainActivity.mWaitForBandResult && !MainActivity.mWaitForConnectACK)
								Cmd4BTConfigThread.sendCommonCmd( BTConfigThread.CMD_QUERY_REPEATER_STATUS,
									MainActivity.mMainAct.mBTConfigHandler);
						...
						//repeater is offline
							...
							//MainActivity.mMainAct.mNeedShowFoundPreBTDialog = false;
							MainActivity.mWaitForBandResult = false;
							MainActivity.mWaitForConnectACK = false;
