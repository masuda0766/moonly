...
#ifdef CONFIG_AUDREY
#if AUDREY_SERVER != 0
static const char *trust_ca_cert = \
"-----BEGIN CERTIFICATE-----\r\n" \
...
char mbedtls_x509_crt_info_buf[1024];
static int my_verify(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
	//char buf[1024];
	((void) data);

	Log_Debug("\r\n[SSL] Verify requested for (Depth %d):\r\n", depth);
	mbedtls_x509_crt_info(mbedtls_x509_crt_info_buf, sizeof(mbedtls_x509_crt_info_buf) - 1, "", crt);
	Log_Debug("\r\n%s", mbedtls_x509_crt_info_buf);
	...
	if(((*flags) & MBEDTLS_X509_BADCERT_REVOKED) != 0) {
		Log_Error("\r\n[SSL]ERROR :   ! server certificate has been revoked");
	}

	if(((*flags) & MBEDTLS_X509_BADCERT_CN_MISMATCH) != 0) {
		Log_Error("\r\n[SSL]ERROR :   ! CN mismatch");
	}

	if(((*flags) & MBEDTLS_X509_BADCERT_NOT_TRUSTED) != 0) {
		Log_Error("\r\n[SSL]ERROR :   ! self-signed or not signed by a trusted CA");
	}

	if(((*flags) & MBEDTLS_X509_BADCRL_NOT_TRUSTED) != 0) {
		Log_Error("\r\n[SSL]ERROR :   ! CRL not trusted");
	}

	if(((*flags) & MBEDTLS_X509_BADCRL_EXPIRED) != 0) {
		Log_Error("\r\n[SSL]ERROR :   ! CRL expired");
	}

	if(((*flags) & MBEDTLS_X509_BADCERT_OTHER) != 0) {
		Log_Error("\r\n[SSL]ERROR :   ! other (unknown) flag");
	}

	if((*flags) == 0) {
		Log_Info("\r\n[SSL]  Certificate verified without error flags\r\n");
	}
...
#ifdef SSL_VERIFY_SERVER
#ifdef CONFIG_AUDREY
	if(mbedtls_x509_crt_parse(_ca_crt, trust_ca_cert, strlen(trust_ca_cert) + 1) != 0)
#else
