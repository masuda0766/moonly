...
#else
#if CONFIG_AUDREY
#define configTOTAL_HEAP_SIZE			( ( size_t ) ( 180 * 1024 ) )	// use HEAP5
#else
...
//#define RTK_MODE_TIMER
	...
	void sh_sys_freertos_task_create_hook(void* pxNewTCB, unsigned long taskNumber, size_t stack_size);	/* void* => TaskHandle_t */
	void sh_sys_freertos_task_delete_hook(void* pxTaskToDelete);	/* void* => TaskHandle_t */
	void sh_sys_freertos_task_switch_in(void* switchInTask, const char *taskName, void *TopOfStack, void *StackArea);	/* void* => TaskHandle_t */


	#define traceTASK_CREATE( pxNewTCB )			sh_sys_freertos_task_create_hook(pxNewTCB, uxTaskNumber,  (usStackDepth * sizeof( StackType_t )));
	#define traceTASK_DELETE( pxTaskToDelete )		sh_sys_freertos_task_delete_hook(pxTaskToDelete);
	#define traceTASK_SWITCHED_IN()					sh_sys_freertos_task_switch_in(pxCurrentTCB, pxCurrentTCB->pcTaskName, pxCurrentTCB->pxTopOfStack, pxCurrentTCB->pxStack);
#endif

#if (CONFIG_AUDREY_SYSDBG == 1)
