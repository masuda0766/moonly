...
// get fast connect type
// return valuse is FAST_CONNECT_TYPE
int get_fast_connect_type(void)
{
	flash_t		flash;
	struct wlan_fast_reconnect *data;
	int ret = FAST_CONNECT_NON;
	...
	if(data){
		...
		if(data->type == FAST_CONNECT_WLAN) {
			Log_Notify("\nConnect type WLAN\n");
			ret = FAST_CONNECT_WLAN;
		} else if (data->type == FAST_CONNECT_BLE){
			Log_Notify("\nConnect type BLE\n");
			ret = FAST_CONNECT_BLE;
		} else {
			Log_Notify("\nConnect type is not set\n");
		}
...
int wlan_init_done_callback()
{
	...
	if(data){
		...
		if(data->type == FAST_CONNECT_WLAN) {
			...
			Log_Info("\n[FAST_CONNECT] Fast connect type is not set, abort fast connection\n");
		}
...
int Erase_Fastconnect_data(){
	...
	flash_erase_sector(&flash, FAST_RECONNECT_BACKUP);
#endif
#if CONFIG_AUDREY_BACKUP_BADGE
	flash_erase_sector(&flash, BADGE_FALSH);
#endif
#if CONFIG_AUDREY_BACKUP_ENTRY
	flash_erase_sector(&flash, ENTRY_FALSH);
#endif
	flash_erase_sector(&flash, FLASH_USER_CONFIG_BT_BASE);
	flash_erase_sector(&flash, FAST_RECONNECT_DATA);
	device_mutex_unlock(RT_DEV_LOCK_FLASH);
	SendMessageToStateManager(MSG_ERASE_CONF_RESET, PARAM_NONE);
	return 0;
}
